...
- Linux
- **An NVIDIA GPU for the retuning campaign.** The five live dashboard sections
  are CPU-only, but the Section 4 retuning campaign is written for the GPU and
  that is how the published run was produced. Verified on an RTX 5070 Ti
  (driver 580.x, CUDA 12 wheels). A CPU fallback exists and is about 4x slower
  per cell.
...
# GPU support for the retuning campaign (~3 GB of CUDA wheels)
...
Confirm the GPU is visible to JAX before starting a long run:

```bash
nvidia-smi                                  # the driver sees the card
./run_retuning_campaign.sh --smoke --plants P001 --drifts D01   # ~1 min
```

The launcher prints `GPU ready: NVIDIA … ` and refuses to start if JAX cannot
reach the device, rather than quietly spending hours on the CPU.
...
### `data/reference_results/` — published results, comparison only

These files hold the previously published result values for each section, so
every table can show the dashboard's own number beside the published one and
the difference between them.

**They are comparison-only and are never an input to any calculation.** Every
dashboard value is recomputed from the model on the machine it runs on; the
reference files are read only to fill the "paper" column of a comparison table.
`backend/validation/paper_reference.py` is the single place they are loaded,
and it is the only module that reads this directory.

Removing the directory is supported: comparison columns then read `—`, the
Tier-1 verification route returns a 422 explaining why, and every study still
computes and reports its own values.

The source documents themselves — the paper, its supplement, the figure
packages and the campaign reports — are **not** part of this repository.
...
| | |
|---|---|
| Hardware | **Runs on the GPU** (`--backend jax`), which is how the published campaign was produced. Add `ALLOW_CPU=1` to fall back to `numpy`. |
| Cost per cell | measured: ~59 s on an RTX 5070 Ti against ~114 s on one CPU core, at the smoke budget |
| Wall time | a few hours for all 120 cells with 6 GPU workers sharing the card |
| Workers | 6 on the GPU (the shape the published campaign used), one per core less one on the CPU; override with `WORKERS=` |
...
The launcher must run on a **GPU node**, not a login node — on a login node
`nvidia-smi` is usually absent and the preflight will tell you so. Request one
first, then run it inside the allocation:

```bash
# Slurm
srun --gres=gpu:1 --cpus-per-task=8 --time=06:00:00 --pty bash
cd /path/to/this/repo && ./run_retuning_campaign.sh

# PBS
qsub -I -l select=1:ncpus=8:ngpus=1 -l walltime=06:00:00
cd /path/to/this/repo && ./run_retuning_campaign.sh
```

Two GPU details the launcher handles for you, both of which otherwise cost
hours:

- **`LD_LIBRARY_PATH`.** `jax[cuda12]` ships its CUDA runtime inside the venv
  and does not add it to the loader path. Without it JAX prints *"Could not
  find cuda drivers on your machine, GPU will not be used"* and silently runs
  on the CPU. The launcher sets it from the installed wheels.
- **Memory across workers.** Each cell is its own process and each initialises
  JAX, which by default preallocates ~75 % of the device — so the second worker
  would fail to allocate. The launcher sets
  `XLA_PYTHON_CLIENT_PREALLOCATE=false` so the workers share the card, which
  also matters on a shared node where someone else already holds memory.

If you invoke `run_tier2_retuning_campaign.py` directly instead of through the
launcher, set both yourself or pass `--backend numpy`.
